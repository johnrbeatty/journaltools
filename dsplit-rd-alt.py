...
# It should be a good starting point for other similar journal segments. This is an alternate version that features
# a slightly different regular expression in the title search. I used it for some scans where I was having trouble
# with the code picking up random paragraphs as titles. So, it limits the search to 100 character titles.
...
def main():
    parser = argparse.ArgumentParser(
        description='Function to extract metadata and split a PDF containing multiple journal articles. Set up for '
                    'Buffalo Law Review recent decisions or case notes, with an alternate title search that limits '
                    'titles to 100 characters.'
    )
